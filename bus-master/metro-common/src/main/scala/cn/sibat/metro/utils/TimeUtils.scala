...
object TimeUtils {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val time1 = "2016-12-01T17:25:11.000Z"
    val time2 = "2016-12-01T17:35:45.000Z"

    val dayDate = timeUtils.date2Stamp(timeUtils.time2Date("2017-01-02", "yyyy-MM-dd"))
    val hourDate = timeUtils.date2Stamp(timeUtils.time2Date(time1.substring(11,19), "HH:mm:ss"))  // 先字符串转时间格式，然后时间转时间戳

    val hourDate1 = timeUtils.date2Stamp(timeUtils.time2Date(time2.substring(11,19), "HH:mm:ss"))
    val timeMid = timeUtils.stamp2Date(hourDate+(hourDate1-hourDate)/2).toString.substring(11,19)
