...
filepath = "E:\\trafficDataAnalysis\\清分比例_8条线修改BUG加深圳北调整（早高峰三分之一）20170421\\工作日-晚高峰-20161102-2016-11-02\\ValidStationPath.txt"
...
for i in range(len(lines)):
    ...
    if line_len ==2:
        ...
        if float(list_prob) != 0:  # 将概率为0 的剔除
            all = "%s,%s,%s,%s,%s,%s,%s" % (str_O, str_D, line0,"","","",list_prob)  # 将字符串按照给定格式连接
            result1.append(all)
    elif line_len ==3:
        str = list_path[1].split(',')
        line0 = str[0]
        ...
        list_prob = record_each[2]
        if float(list_prob) != 0:  # 将概率为0 的剔除
            all = "%s,%s,%s,%s,%s,%s,%s" % (str_O, str_D, line0, line1, "", "", list_prob)  # 将字符串按照给定格式连接
            result2.append(all)
    elif line_len ==4:
        str = list_path[1].split(',')
        line0 = str[0]
        ...
        list_prob = record_each[2]
        if float(list_prob) != 0:  # 将概率为0 的剔除
            all = "%s,%s,%s,%s,%s,%s,%s" % (str_O, str_D, line0, line1, line2, "", list_prob)  # 将字符串按照给定格式连接
            result3.append(all)
    elif line_len == 5:
        str = list_path[1].split(',')
        line0 = str[0]
        ...
        list_prob = record_each[2]
        if float(list_prob) != 0:  # 将概率为0 的剔除
            all = "%s,%s,%s,%s,%s,%s,%s" % (str_O, str_D, line0, line1, line2, line3, list_prob)  # 将字符串按照给定格式连接
            result4.append(all)

str_result = ''.join(result1)
path = "C:\\Users\\will\\Desktop\\workday\\wan\\workday_1.txt"
f = open(path, 'w')
f.write(str_result)
f.close()

str_result1 = ''.join(result2)
path1 = "C:\\Users\\will\\Desktop\\workday\\wan\\workday_2.txt"
f1 = open(path1, 'w')
f1.write(str_result1)
f1.close()

str_result2 = ''.join(result3)
path2 = "C:\\Users\\will\\Desktop\\workday\\wan\\workday_3.txt"
f2 = open(path2, 'w')
f2.write(str_result2)
f2.close()

str_result3 = ''.join(result4)
path3 = "C:\\Users\\will\\Desktop\\workday\\wan\\workday_4.txt"
f3 = open(path3, 'w')
